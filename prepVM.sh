...
echo
echo oc login --insecure-skip-tls-verify=false -u admin -p ADMIN_PW  https://api.cluster-${CLUSTERID}.dyn.redhatworkshops.io:6443
oc login --insecure-skip-tls-verify=false -u admin -p $ADMIN_PW  https://api.cluster-${CLUSTERID}.dyn.redhatworkshops.io:6443
